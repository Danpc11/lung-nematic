"""Local sensitivity rank analysis and precision bounds for the IPF twin.

What this module actually computes
----------------------------------
It builds the sensitivity matrix of the observation vector with respect to the
free parameters, evaluated **at one nominal parameter point** by finite
differences, and reports its numerical rank. That answers a *local* question:
whether the model is identifiable in a neighbourhood of that point, for that
design.

It is important not to overstate this. Full rank at a point establishes **local**
identifiability there, not global structural identifiability: discrete
symmetries, disconnected solution branches, and regions of parameter space where
rank collapses are all invisible to a single-point Jacobian. Proving global
structural identifiability needs symbolic work - differential algebra, a full
observability rank condition, or exhaustive profiling over the plausible region
from multiple starts. This module does none of that, and its API is named
accordingly (``is_locally_identifiable``, not ``is_structurally_identifiable``).

Symmetrically, a rank deficiency found at one point is evidence of a locally
flat direction there, not proof that the direction is flat everywhere. Some of
the deficiencies this model exhibits *are* exact and global - when treatment
status never changes, ``beta`` literally does not enter the equations, so no
data of that design can recover it - but that is a fact about this model that a
human verified, not something the rank computation established. Pass
``n_probes`` to re-evaluate the rank at perturbed parameter points; a rank that
is stable across the plausible region is much stronger evidence than a rank at
one point, while still falling short of proof.

Precision, given local identifiability
--------------------------------------
Where the information matrix is nonsingular, the Cramer-Rao bound says how
precisely parameters can be recovered given a realistic number of visits and
pinned measurement noise. A model can be locally identifiable and still useless
if the standard error on ``r_i`` exceeds the effect you need to detect, and that
is the question that decides how many visits a study needs.

The bound is also local, and this model has a global failure mode it cannot see;
see ``monte_carlo_check``.

Run both before writing an estimator. A precision bound computed where the
information matrix is singular is meaningless.
"""
...
class IdentifiabilityReport:
    """Result of a local sensitivity rank analysis at a nominal parameter point.

    ``probe_ranks`` holds the ranks recomputed at randomly perturbed parameter
    points, when ``n_probes`` was requested. It is the difference between "full
    rank at the value I happened to pick" and "full rank everywhere I looked".
    """
    ...
    probe_ranks: list[int] | None = None

    @property
    def is_locally_identifiable(self) -> bool:
        """Full rank **at the nominal point**. Not a global claim."""
        return self.rank == len(self.free_names)

    @property
    def rank_is_stable(self) -> bool | None:
        """Whether every probe point gave the same rank. ``None`` if unprobed."""
        if not self.probe_ranks:
            return None
        return all(rank == self.rank for rank in self.probe_ranks)
...
def analyze(
    ...
    *,
    rank_tol: float = 1e-8,
    n_probes: int = 0,
    probe_scale: float = 0.3,
    probe_seed: int = 0,
) -> IdentifiabilityReport:
    """Local sensitivity rank analysis plus precision bounds for one design.

    Parameters
    ----------
    n_probes:
        Recompute the rank at this many randomly perturbed parameter points.
        The rank reported in ``rank`` is always the one at ``params``; the probe
        ranks say whether that finding survives across the plausible region.
        A rank that is full at the nominal point but collapses at a probe is the
        signal that the nominal value sat on a non-generic point, and it is
        exactly the case a single-point Jacobian would have reported as clean.
    probe_scale:
        Relative log-normal spread of the perturbations.
    """
    ...
    if n_probes > 0:
        ...
        for _ in range(n_probes):
            # Multiplicative jitter keeps signs and orders of magnitude, which
            # is what "elsewhere in the plausible region" means for rate and
            # scale parameters. Additive jitter would happily propose a negative
            # decline rate and test a region nobody cares about.
            jittered = base * np.exp(rng.normal(0.0, probe_scale, base.shape))
            probe = params.with_values(free_names, jittered)
            probe_matrix = sensitivity_matrix(probe, free_names, schedule, mode)
            probe_spectrum = np.linalg.svd(probe_matrix, compute_uv=False)
            top = probe_spectrum[0] if probe_spectrum.size else 1.0
            probe_ranks.append(
                int((probe_spectrum > rank_tol * top).sum()) if top > 0 else 0
            )
    ...
    return IdentifiabilityReport(
        ...
        nominal={name: float(getattr(params, name)) for name in free_names},
    ...
    *,
    ...
    for n_visits in range(2, max_visits + 1):
        try:
            ...
            # Short schedules can end before treatment starts, which is not an
            # error in the search - it just means this visit count cannot
            # represent the requested design. Keep going.
            continue
